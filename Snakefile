...
ont_illumina_hybrid_assemblies = expand("{ont}/hybrid_{illumina}/{assembler}",
    ont = [r["name"] for r in config["ont"]],
    illumina =  [r["name"] for r in config["illumina"]],
...
rule all :
	...
	# For each run-assembler pair
        ...
        expand("outputs/{assembly}/bins_metaquast_results/summary/TSV/", assembly = assemblies)
            if(config["metrics"] and ("bin-reference-based" in config["metrics"])) else "Snakefile",
        #"test.txt",
